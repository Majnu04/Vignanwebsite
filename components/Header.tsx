...
const SubMenuItem: React.FC<SubMenuItemProps> = ({ item, onHover }) => {
    ...
    return (
        <li
            className={`group/submenu-item relative rounded-lg transition-colors duration-200 hover:bg-slate-100 ${isSubOpen ? 'z-30' : 'z-auto'}`}
            onMouseEnter={() => { onHover(); if (hasNestedSubmenu) setIsSubOpen(true); }}
            onMouseLeave={() => { if (hasNestedSubmenu) setIsSubOpen(false); }}
        >
            {item.target || item.link.startsWith('http') || item.link === '#' ? (
                <a href={item.link} target={item.target} className="flex w-full items-center justify-between p-3 text-sm">
                    <span className="absolute -left-3 top-1/2 h-2/3 w-1 rounded-full bg-slate-400 transition-all duration-300 ease-in-out transform -translate-y-1/2 scale-y-0 group-hover/submenu-item:scale-y-100"></span>
                    <span className="font-medium text-slate-700 group-hover/submenu-item:text-slate-900">{item.name}</span>
                ...
                <Link to={item.link} className="flex w-full items-center justify-between p-3 text-sm">
                    <span className="absolute -left-3 top-1/2 h-2/3 w-1 rounded-full bg-slate-400 transition-all duration-300 ease-in-out transform -translate-y-1/2 scale-y-0 group-hover/submenu-item:scale-y-100"></span>
                    <span className="font-medium text-slate-700 group-hover/submenu-item:text-slate-900">{item.name}</span>
                    {hasNestedSubmenu && <svg className="h-4 w-4 text-slate-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d={opensDown ? "M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" : "M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"} clipRule="evenodd" /></svg>}
                </Link>
            ...
            {hasNestedSubmenu && (
                ...
                >
                    <ul 
                        className={`bg-white rounded-xl shadow-2xl shadow-slate-800/20 border border-slate-200/60 p-2 space-y-1 ${item.isScrollable ? 'custom-scrollbar' : ''}`}
...
const Header: React.FC<HeaderProps> = ({ onLogoClick }) => {
    ...
    return (
        <>
            ...
            `}</style>
            <header className="fixed top-0 w-full z-50 transition-all duration-300 ease-in-out bg-white" onMouseLeave={handleMouseLeave}>
                <div className="absolute top-0 left-0 right-0 h-full transition-all duration-300" style={{ background: isScrolled ? 'rgba(255, 255, 255, 1)' : 'rgba(255, 255, 255, 1)', backdropFilter: 'none', boxShadow: isScrolled ? '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1)' : 'none' }}></div>
                        ...
                        <nav className="hidden lg:flex lg:items-center lg:h-full lg:space-x-1">
                            {navItems.map((item) => (
                                <div key={item.name} className="h-full flex items-center justify-center transition-transform duration-300 hover:-translate-y-0.5" onMouseEnter={() => handleMouseEnter(item)}>
                                    {item.target || item.link.startsWith('http') ? (
                                        <a href={item.link} target={item.target} className={`relative flex items-center px-4 py-2 font-medium rounded-lg whitespace-nowrap font-['Georgia',_serif] transition-all duration-300 ${activeMenu?.name === item.name && item.sub ? 'bg-slate-200 text-blue-600' : "text-slate-700 hover:bg-slate-200 hover:text-blue-600"}`}>
                                            <span>{item.name}</span>
                                        ...
                                        <Link to={item.link} className={`relative flex items-center px-4 py-2 font-medium rounded-lg whitespace-nowrap font-['Georgia',_serif] transition-all duration-300 ${activeMenu?.name === item.name && item.sub ? 'bg-slate-200 text-blue-600' : "text-slate-700 hover:bg-slate-200 hover:text-blue-600"}`}>
                                            <span>{item.name}</span>
                                            {item.sub && <svg className={`h-4 w-4 ml-1.5 transition-transform duration-300 ${activeMenu?.name === item.name ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg>}
                                        </Link>
                                    )}
                ...
                <div onMouseEnter={handleMenuContainerEnter} className={`absolute top-full left-0 right-0 z-10 transition-all duration-300 ease-in-out transform-gpu ${activeMenu ? 'opacity-100 visible translate-y-0 scale-100' : 'opacity-0 invisible -translate-y-3 scale-98'}`}>
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="bg-white rounded-b-2xl shadow-2xl shadow-slate-800/20 border-t border-slate-200/60 overflow-hidden">
                                ...
                                <div className="flex">
                                    <div className={`flex flex-1 gap-x-8 p-10 justify-start`}>
                                        {activeMenu.sub.map((column, index) => (
                                            <div 
                                                key={column.name} 
                                                className="px-5 border-l border-slate-200 first:border-l-0" 
                                            ...
                                            >
                                                <h3 className="text-base font-semibold text-slate-800 mb-5 font-['Georgia',_serif]">{column.name}</h3>
                                    ...
                                    {activeMenu.isMegaWithImage && (
                                        ...
                                        >
                                            <div className="aspect-w-4 aspect-h-3 rounded-xl overflow-hidden shadow-md border border-slate-200 w-full h-full">
